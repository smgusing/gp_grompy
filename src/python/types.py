## version 0.10
# A large portion of this file is copied from GromPy
...
from gp_grompy import c_real,rvec,ivec,tensor,matrix,splinevec,dvec
...
class t_iparams(Union):
    ...
    class harmonic(Structure):
        _fields_ = [("rA", c_real),
                    ...
                    ("rb", c_real),
                    ("krB", c_real)]
    class restraint(Structure):
        _fields_ = [("lowA",c_real),
                    ("up1A",c_real),
                    ("up2A",c_real),
                    ("kA",c_real),
                    ("lowB",c_real),
...
#} cmapdata_t;
class cmapdata_t(Structure):
    _fields_ =[("cmap",POINTER(c_real))]
...
#} gmx_cmap_t;
class gmx_cmap_t(Structure):
    _fields_ =[("ngrid",c_int),
               ("grid_spacing",c_int),
...
#} t_resinfo;
class t_resinfo(Structure):
    _fields_ =[("name",POINTER(c_char_p)),
               ("nr",c_int),
               ("ic",c_ubyte),
               ("chainnum",c_int),
               ("chainid",c_char),
               ("rtp",POINTER(c_char_p))]
#
#typedef struct {
#  int  type;                    /* PDB record name                      */
#  int  atomnr;                  /* PDB atom number                      */
...
class t_atomtypes(Structure):
    _fields_ = [("nr", c_int),
                ...
                ("gb_radius", POINTER(c_real)),
                ("S_hct", POINTER(c_real)),
...
#} mdrunner_comm;
#########NEW STRUCUTURES###################3
class t_gmxvmdplugin(Structure):
    pass
...
class output_env_t(Structure):
    pass

class t_trxstatus(Structure):
    pass

class t_fileio(Structure):
    pass
class gmx_rmpbc_t(Structure):
    pass
